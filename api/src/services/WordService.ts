...
class WordService {
  ...
  static getWordList = (startingTeam: Team): Word[] => {
    ...
    for (let i = 0; i < 9; i++) {
      if (i < 7) assignments.push(WordAssignment.Neutral);
      if (i < 8)
        assignments.push(startingTeam === Team.Blue ? WordAssignment.Red : WordAssignment.Blue);
      assignments.push(startingTeam === Team.Blue ? WordAssignment.Blue : WordAssignment.Red);
    }
    assignments = WordService.shuffleArray(assignments);

    for (let i = 0; i < list.length; i++) {
      list[i].assignment = assignments[i];
    }
